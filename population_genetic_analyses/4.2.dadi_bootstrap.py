# random sets of the original CriolloCuraray.dadi were created by sampling lines with replacement from the file to generate 1000 "dadi_boot_data" files of the same size (number of lines as the original)
...
dd = dadi.Misc.make_data_dict("dadi_boot_data")
...
import numpy
numpy.savetxt('dadi_boot.Expected_folded.txt', expected_folded)
#numpy.savetxt('dadi_boot.Expected.txt', expected)
numpy.savetxt('dadi_boot.Popt.txt', popt)
numpy.savetxt('dadi_boot.ResidualsAnscombe.txt', residAnscombe)
numpy.savetxt('dadi_boot.ResidualsLinear.txt', residLinear)
ll_optF2 = array([ll_optF])
numpy.savetxt('dadi_boot.llopt.txt', ll_optF2)
thetaF2 = array([thetaF])
numpy.savetxt('dadi_boot.theta.txt', thetaF2)
